"""
Turns experiment_single_star.py's raw results into the 11x11 region map,
purely by clustering *which registers respond* to a star at each position -
no assumption about grid shape or region boundaries going in.

  - A register that differs from the all-zero baseline at every position
    sharing the same value mod 11 (checked for all 11 residues) is that
    column's counter - this is what establishes the scan is 11-wide and
    row-major in the first place, not assumed beforehand.
  - No register turned out to differ over exactly 11 *consecutive*
    positions (a hypothetical per-row counter) - consistent with row
    validity being tracked by a single reused current-row tally rather
    than eleven persistent counters (columns and regions need persistent
    counters because their cells are scattered across the whole 121-cycle
    scan; a row's cells are all processed back-to-back, so one register
    that resets each row boundary suffices).
  - The remaining registers cluster into exactly 11 groups by the set of
    positions they respond to, with sizes {4,5,6,7,8,8,9,11,14,21,28} -
    both the count (11) and the size multiset match "eleven regions" and
    together cover all 121 positions exactly once.
"""
import json
import os
...
def main():
    ...
    print(f"column counters found: {len(col_regs)}/11")
    ...
    # Principled filter, not a fit to a size list we already expect: regions
    # must *partition* the 121 cells (11 pairwise-disjoint sets that union
    # to everything) - that's intrinsic to what "regions" means, not
    # borrowed from any known answer. Exact-cover search over the
    # candidates for such a set of 11.
